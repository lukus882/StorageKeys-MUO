...
namespace Server.Gumps
{
    ...
    public class CliLocViewerGump : Gump
    {
        ...
        public CliLocViewerGump(Mobile owner,string filtertext,int page) : base(10,10)
        {
            ...
            _Owner.CloseGump<CliLocViewerGump>();
        ...
        protected void DrawEntries()
        {
            ...
            for (int i = _Start; i < _End; i++)
            {
                if (enumerator.MoveNext())
                {
                    if (i == _Start)
                    {
                        if (enumerator.Key is int key)
                        {
                            _TopNumber = key;
                        }
                        else
                        {
                            Console.WriteLine($"Invalid cast: {enumerator.Key.GetType()} cannot be cast to int.");
                        }
                    }

                    // Check and cast the key to string
                    if (enumerator.Key is int keyLabel)
                    {
                        AddLabel(_X, _Y += 20, 1152, keyLabel.ToString());
                    }
                    else
                    {
                        Console.WriteLine($"Invalid cast: {enumerator.Key.GetType()} cannot be cast to int.");
                    ...
                    // Check and cast the value to string
                    if (enumerator.Value is string valueLabel)
                    {
                        AddLabel(_X + 150, _Y, 1152, valueLabel);
                    }
                    else
                    {
                        Console.WriteLine($"Invalid cast: {enumerator.Value.GetType()} cannot be cast to string.");
                    }
                }
            }
        }
        ...
        protected void AddPageButtons()
        {
            ...
            AddLabel(_Width / 2 - 10,_Y,88,$"({_Page + 1}/{_MaxPages})");
        }
        ...
        {
            var relay = info.GetTextEntry(index);
            return relay == null ? null : relay.Trim();
        }
        ...
        protected static Hashtable GetFilteredCliLocs(string filterstring)
        {
            ...
            while (enumerator.MoveNext())
            {
                ...
                if (enumerator.Value is string value && value.ToLower().Contains(filterstring.ToLower()))
                {
                    // Ensure the key is an integer before casting
                    if (enumerator.Key is int key)
                    {
                    ...
                    {
                        Console.WriteLine($"Invalid cast: Key = {enumerator.Key.GetType()} cannot be cast to int.");
                    }
                }
            }
